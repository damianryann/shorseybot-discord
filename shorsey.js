...
client.on('message', msg => {
  ...
  const comingResponse = shuffle([
    ...
    `${msg.author} this is a test.`
  ]);

  const whatHappens = shuffle([
    `${msg.author} Three things: I hit you, you hit the pavement, ambulance hits 60.`,
    `${msg.author} Three things: I hit you, you hit the pavement, and I jerk off on your driver’s side door handle.`
  ]);
...
client.on('message', msg => {
  ...
  if (msg.content === `I'm coming`) {
    msg.channel.send(
      comingResponse[Math.floor(Math.random() * comingResponse.length)]
    );
  }

  if (msg.content === `What's going to happen?`) {
    msg.channel.send(
      whatHappens[Math.floor(Math.random() * whatHappens.length)]
    );
  }
});
